      ...
      <div className="bg-white dm-surface w-full max-w-md rounded-2xl shadow-2xl dm-glow overflow-hidden">
        ...
        <div className="p-8">
            <form onSubmit={submit} className="space-y-5">
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">{mode === 'login' ? 'Adresse email' : 'Email'}</label>
                    <input 
                        type="text" 
                        required 
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors"
                        placeholder={mode === 'login' ? 'vous@exemple.com (ou admin)' : 'vous@exemple.com'} 
                        value={form.email} 
